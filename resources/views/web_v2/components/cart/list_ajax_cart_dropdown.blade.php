...
@else
	<li class=" solid text-center">
		<h4 class="pt-md pb-md mt-0 text-md" style="text-transform:none; font-weight: 300; letter-spacing: 0.04em;">Belum ada item di Cart</h4>
	</li>
	...
	<?php
		...
		for ($x=1; $x<3; $x++) 
		{
			$recommendation[$x]['id']			= $x;
			$recommendation[$x]['thumbnail'] 	= $image[array_rand($image)];
			$recommendation[$x]['name']			= $name[array_rand($name)];
			$recommendation[$x]['prices']		= $price[array_rand($price)];
			$recommendation[$x]['slug']			= str_slug($recommendation[$x]['name'], '-');
		}
	...
	@foreach($recommendation['data'] as $k => $item)
		<li class="{{ (count($recommendation['data']) > ($k+1)) ? 'border-bottom' : '' }}">
