	...
	<body class="@yield('body_class')" style="background-color: #f5f5f5;">
		...
		<div class="navbar navbar-default navbar-fixed-bottom navbar_shortcut hidden-lg hidden-md hidden-sm col-xs-12 border-top-1 border-grey" role="navigation">
			<div class="nav navbar-nav text-center mt-0 mb-0">
				...
				<div onclick="location.href='{{ URL::route('balin.product.index') }}';" class="col-xs-{{ (Session::has('whoami')) ? "3" : "4" }} text-center border-right-1 border-grey pt-xs pb-xs">
				...
				@endif
				@if (Session::has('whoami'))
					<div onclick="location.href='{{ URL::route('balin.get.logout') }}';" class="col-xs-3 text-center pt-xs pb-xs">
						{!! HTML::image('images/sign-out.png', 'image' ,['style' => 'height:35px; width:25px; margin: 0 auto;', 'class' => 'pt-5 pb-5']) !!}
				@else
					<div onclick="location.href='{{ URL::route('balin.get.login') }}';" class="col-xs-4 text-center pt-xs pb-xs">
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				console.log($('.wrapper_content').height());
				console.log($('.divider_footer').position().top);
				wrapper_height = $('.wrapper_content').height();
				$(function () {

					$(window).scroll( function() {
						if ($(this).scrollTop() >= wrapper_height) {
